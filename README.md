...
* **[특이사항] 하드웨어 구현 대체 조건**: 본 제품의 최종 규격은 라즈베리파이 전용 카메라 모듈을 탑재하는 것이나, **현재 개발 환경의 물리적인 부품 수급 상황(카메라 모듈 부재)으로 인해, 이번 프로토타입 구현 및 기능 검증 단계에 한하여 스마트폰 카메라(IP Webcam 연동 방식)로 사진 촬영 기능을 임시 대체**하여 시스템을 실증함.
...
graph TD
    ...
    C1 --> D1{사람이 감지되었는가?}
    D1 -- Yes --> E1[임시 대체: 스마트폰 카메라 스트림에서 프레임 캡처]
...
| **FR-02** | PIR 연동 카메라 제어 | 평시에는 카메라 촬영을 제한하고, PIR 센서에 움직임 신호(`HIGH`)가 올 때만 연동 장치(스마트폰 카메라 스트림)에서 즉시 사진을 캡처할 것. | **Must-Have** |
...
* **성능 (Performance)**
  * PIR 센서 감지 시점부터 스마트폰 스트림 프레임 캡처, TFLite 안면 인식 연산 완료 및 FastAPI 서버 저장까지의 총 걸리는 시간은 **1.5초 이내**여야 함.
