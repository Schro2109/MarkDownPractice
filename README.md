...
* **1~2주차:** 라즈베리파이 환경 설정, 초음파 및 PIR 센서 회로 구성 및 Python 센서 계측 코드 작성.
* **3~4주차:** 로컬 MQTT Broker 구축 및 센서-FastAPI 간 메시지 송수신 연동 테스트. OpenCV 기반 얼굴 인식 모델 최적화 및 집주인 사진 등록 프로세스 구현.
* **5~6주차:** 외부 인터넷망 연동 및 WebSocket 기반 스마트폰 실시간 알림 연동, 모바일 뷰 UI 구현 및 최종 통합 테스트.
* **독립 테스트 및 디버깅 방안:** 로컬 터미널을 분리하여 `mosquitto_sub -t home/sensor` 명령어를 통해 백엔드 서버가 켜지지 않은 상태에서도 하드웨어에서 센서 데이터가 유실 없이 실시간으로 발행(Publish)되는지 교차 검증 진행.
